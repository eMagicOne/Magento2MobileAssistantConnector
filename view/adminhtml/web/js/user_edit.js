...
require([
    ...
    var password           = $('#user_username');
    var login              = $('#user_password');
    var password_value_old = password.val();
    var login_value_old    = login.val();

    // Need to load QR-code correct
    setTimeout(
        function() {
            var entryPoint = $("#qrcode_image-note");
            var qrCodeLink = $("#user_qrcode_link");
            ...
            if (entryPoint.length > 0) {
                ...
                qrCode.makeCode($('#user_qr_code_data').val());

                $("#mobassistantconnector_qr_code_container").append(
            ...
            if (qrCodeLink.length > 0) {
                ...
                $("#user_qrcode_hash").val() + "' readonly></div>" +
            ...
            $("#mobassistantconnector_qr_code_link_btn_regenerate").click(function() {
                $("#mobassistantconnector_qr_code_link_hash").val(SHA256("'" + $.now() + "'"));
                $("#user_qrcode_link").hide();
                $("#mobassistantconnector_qr_code_link_save_data").show();
            });
        },
        100
    );
    ...
    $("#saveandcontinue").click(function() {
        removeClass($("#deviceGrid_massaction-select"), "required-entry");
    });
    $("#save").click(function() {
        removeClass($("#deviceGrid_massaction-select"), "required-entry");
    });
    ...
    function changeQRCode() {
        ...
        var qrcode_changed = $('#mobassistantconnector_qrcode_changed');
        var qrcode_img     = $('#mobassistantconnector_qr_code_img');
